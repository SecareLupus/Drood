...
Section "-InstallStartCleanup"
  ...
  ${CleanUpdatesDir} "Greyfirst\Drood"
...
Section "-Application" APP_IDX
  ...
  WriteRegStr HKLM "$0\drood" "" "URL:Drood Protocol"
  WriteRegStr HKLM "$0\drood" "URL Protocol" ""
  WriteRegStr HKLM "$0\drood\DefaultIcon" "" "$INSTDIR\${FileMainEXE},0"
  WriteRegStr HKLM "$0\drood\shell\open\command" "" "$\"$INSTDIR\${FileMainEXE}$\" $\"%1$\""

  WriteRegStr HKLM "$0\droods" "" "URL:Secure Drood Protocol"
  WriteRegStr HKLM "$0\droods" "URL Protocol" ""
  WriteRegStr HKLM "$0\droods\DefaultIcon" "" "$INSTDIR\${FileMainEXE},0"
  WriteRegStr HKLM "$0\droods\shell\open\command" "" "$\"$INSTDIR\${FileMainEXE}$\" $\"%1$\""

  WriteRegStr HKLM "$0\.drood" "" "DroodProject"
  WriteRegStr HKLM "$0\.tdrood" "" "DroodTemplate"

  WriteRegStr HKLM "$0\DroodProject" "" "Drood Document"
  WriteRegStr HKLM "$0\DroodProject\DefaultIcon" "" "$INSTDIR\${FileMainEXE},1"
  WriteRegStr HKLM "$0\DroodProject\shell\open\command" "" "$\"$INSTDIR\${FileMainEXE}$\" $\"%1$\""

  WriteRegStr HKLM "$0\DroodTemplate" "" "Drood Template"
  WriteRegStr HKLM "$0\DroodTemplate\DefaultIcon" "" "$INSTDIR\${FileMainEXE},1"
  WriteRegStr HKLM "$0\DroodTemplate\shell\open\command" "" "$\"$INSTDIR\${FileMainEXE}$\" $\"%1$\""
  ...
  ${If} $TmpVal == "HKLM"
    ...
    StrCpy $0 "Software\Greyfirst\Drood\InstallInfo"
