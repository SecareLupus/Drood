...
Section "Uninstall"
  ...
  ${If} "$0" == "1"
    ${un.DeleteRelativeProfiles} "Greyfirst\Drood"
    RmDir "$APPDATA\Greyfirst\Extensions\drood@drood.com"
  ...
  ${un.RegCleanAppHandler} "DroodProject"
  ${un.RegCleanAppHandler} "DroodTemplate"

  ClearErrors
  ReadRegStr $R9 HKCR "DroodProject" ""
  ; Don't clean up the file handlers if the DroodProject key still exists since
  ; there should be a second installation that may be the default file handler
  ${If} ${Errors}
    ${un.RegCleanFileHandler}  ".drood"  "DroodProject"
    ${un.RegCleanFileHandler}  ".tdrood" "DroodTemplate"
  ${EndIf}
  ...
  ; installing even if there is another install of Drood that is set as the
  ...
  ${If} $R9 == "false"
    ...
    DeleteRegKey HKLM "$0"
    DeleteRegKey HKCU "$0"
    StrCpy $0 "Software\Classes\MIME\Database\Content Type\application/x-xpinstall;app=drood"
    DeleteRegKey HKLM "$0"
    DeleteRegKey HKCU "$0"
  ${Else}
    ReadRegStr $R1 HKLM "$0" ""
  ...
  ${un.CleanUpdatesDir} "Greyfirst\Drood"
  ...
  ; If drood.exe was successfully deleted yet we still need to restart to
  ; remove other files create a dummy drood.exe.moz-delete to prevent the
