...
function getProjectFile () {
  if (! window._droodfiledesc)
    return null;
  try {
    var file = Components.classes["@mozilla.org/file/local;1"]
      .createInstance(Components.interfaces.nsILocalFile);
    file.persistentDescriptor = window._droodfiledesc;
    return file;
  }
  catch (ex) {
    return null;
...
function setProjectFile (file) {
  if (file && file instanceof Components.interfaces.nsILocalFile) {
    try {
      ...
      window._droodfiledesc = file.persistentDescriptor;
    }
    catch (ex) {
      window._droodfiledesc = null;
    }
  }
  else {
    window._droodfiledesc = null;
...
function loaded () {
  ...
  obsvc.addObserver(gController, "drood:login-status-changed", false);
  obsvc.addObserver(gController, "drood:recent-projects-changed", false);
  obsvc.addObserver(gController, "drood:notification-count-changed", false);

  var cxsvc = getDroodService();
  ...
  if (window.arguments && window.arguments.length > 0) {
    ...
    else if (openarg.match(/^droods?:\/\//))
      setTimeout(function () { loadDroodURI(openarg); }, 0);
    else
...
function showDevWarningIfNeeded () {
  ...
  var prefs = getPrefService().getBranch("drood.server.");
...
function updateStatusbar () {
  ...
  if (cxsvc.loggedIn) {
    ...
    networkmenu.setAttribute("image", "chrome://drood/skin/online.png");
  }
  else {
    loginmenuitem.hidden = false;
    logoutmenuitem.hidden = true;
    networkmenu.label = gApp.getText("SignedOut");
    networkmenu.setAttribute("image", "chrome://drood/skin/offline.png");
  }
  updateNotifierLabel();
}
...
function loadServerItem (aItem) {
  if (! aItem.fetchURL && aItem.exportURL) {
    ...
    xhr.onload = function () {
      var url = xhr.responseText.replace(/^drood/, "http");
    ...
    xhr.onerror = function () {
      ...
      droodBugAlert(ex, Components.stack, ex);
    };
    xhr.send(null);
    return;
  }
  ...
  try {
    ...
    if (aItem.type == "project")
      tmpfile.append("download-0.drood");
    ...
    var listener = {
      ...
      onStateChange: function (prog, request, stateFlags, status) {
        ...
        try {
          ...
          if (! this.wsref) {
            try {
              this.wsref = channel.getResponseHeader("X-Drood-WSRef");
            }
            catch (ex) {}
          }
        }
...
function loadDroodURI (uri) {
  try {
    var tmpfile = getTempDir();
    var filename = "drood-download-0.cxext";
    ...
    if (listener.succeeded) {
      ...
      getDroodService().checkLoginFromCookie();

      switch (listener.type) {
        case "application/x-drood-project":
          loadProject(fileToFileURL(dstfile), true, listener.wsref);
          break;
        case "application/x-drood-extension":
...
function loadProjectPreflight () {
  ...
  var branch = ps.getBranch("drood.spelling.");
  ps = ps.QueryInterface(Components.interfaces.nsIPrefBranch2);
  ps.addObserver("drood.spelling.", gController, false);
...
function loadProjectPostflight () {
  ...
  try {
    var ps = getPrefService();
    ps.addObserver("drood.autosave", gController, false);
    var saveInterval = getPrefService().getIntPref("drood.autosave");
    if (saveInterval > 0) {
 ...
 *
 * @param projuri  a file uri referring to a .drood file (or a project.rdf file)
...
function loadProject (projuri, isremote, wsref) {
  ...
  try {
    ...
    // extractDroodProject reverts to using zipped .drood files, this
    // needs to be special-cased.
    if (projuri.match(/\.(t?drood|rdf)$/)) {
      droodfile = fileURLToFile(projuri);
      if (! (droodfile.leafName.match(/\.t?drood$/) && droodfile.fileSize > 0))
        droodfile = null;
      projuri = extractDroodProject(projuri);
    }

    setProjectFile(droodfile);
    ...
    // If we weren't launched from a .drood file, check if conversion needed
    if (doConvertCheck) {
    ...
    if (gProject.isTemplate) {
      ...
      if (! (droodfile && droodfile.parent.equals(samplesFolder))) {
      ...
      if (localfile && ! isremote) {
        setWindowTitle(localfile.leafName.replace(/\.t?drood$/, ""));
...
function loadScript (scriptFile, serverItem) {
  ...
  try {
    ...
    try {
      ...
      droodBugAlert(gApp.getText("UnknownErrorMsg"), Components.stack, ex);
      return;
    }
    finally {
...
function delayedAlertCallback () {
  droodBugAlert(gDelayedAlertMsg, gDelayedAlertStack, gDelayedAlertException);
}
...
var gController = {
  ...
  observe: function (subject, topic, data) {
    if (topic == "nsPref:changed") {
      if (data == "drood.spelling.inline") {
        var ps = getPrefService().getBranch("drood.spelling.");
      ...
      else if (data == "drood.autosave") {
        var ps = getPrefService().getBranch("drood.");
        ...
        if (saveInterval > 0) {
          gWindow.autosaveTimer = setInterval("autosave()",
            saveInterval * 60000); // measured in minutes
             ...
             topic == "drood:login-status-changed" ||
             topic == "drood:notification-count-changed") {
      updateStatusbar();
    }
    else if (topic == "drood:recent-projects-changed") {
  ...
  _frameFinishedLoadingImpl: function _frameFinishedLoadingImpl (aDocURI) {
    ...
    if (frame) {
      ...
      try {
        ...
        droodBugAlert(msg, Components.stack, ex);
      }
    }
    else {
  ...
  loadNextDocument: function loadNextDocument () {
    ...
    */
    ...
    try {
      ...
      droodBugAlert(gApp.getText("UnableToOpenItemMsg"), Components.stack, ex);
...
var gDocDNDObserver = {
  ...
  attemptExternalDrop: function attemptExternalDrop (row, orientation,
    ...
    try {
      ...
      droodBugAlert(gApp.getText("UnableToCopy"), Components.stack, ex);
    }
    gProject.ds.endUpdateBatch();
    gProject.isModified = true;
  },
...
function extractDroodProject (fileuri) {
  ...
  if (file.leafName.match(/\.t?drood$/) && file.fileSize > 0) {
  ...
  // Otherwise, assume we're opening a .rdf or .drood in a project folder
...
function projectWindowWillClose () {
  ...
  try {
    ...
    obsvc.removeObserver(gController, "drood:login-status-changed");
    obsvc.removeObserver(gController, "drood:recent-projects-changed");
    obsvc.removeObserver(gController, "drood:notification-count-changed");
    ...
    if (gProject) {
      ...
      try {
        ...
        var branch = ps.getBranch("drood.spelling.");
        ps = ps.QueryInterface(Components.interfaces.nsIPrefBranch2);
        ps.removeObserver("drood.spelling.", gController);
        ps.removeObserver("drood.autosave", gController);
      }
      catch (ex) {
 ...
 * as a .drood file, this will prompt the user to save the project and initiate
 * a save if necessary.
 *
 * @type boolean
...
function tryToCloseProject (reason) {
  ...
  // modified.
  var obsvc = getObserverService();
  obsvc.notifyObservers(gProject, "drood:project-window-closing", null);
...
function addToTemplates (droodfile) {
  var location = currentProfileDir();
  location.append(Cx.TEMPLATES_DIR);
  copyToUnique(droodfile, location, droodfile.leafName, true);
}
...
function cmdCopyDocumentAs (event) {
  ...
  if (event.target.value != Cx.NS_CX + "StoryboardDocument2") {
    ...
    try {
      ...
      droodBugAlert(gApp.getText("CopyDocumentFailed"), Components.stack, ex);
    }
    gProject.ds.endUpdateBatch();
...
function cmdCopyProject () {
  try {
    ...
    var projuri = copyToUnique(gProject.projectFolder,
      gProject.projectFolder.parent, "drood_project");
...
function convertProject (fileURL, convertURL) {
  ...
  var droodFile = projDir.clone();
  droodFile.append(projDir.leafName + '.drood');
  if (! droodFile.exists()) {
    droodFile.create(0, 0600);
  }
}
...
function notificationToPopupContent (notification) {
  ...
  image.setAttribute("src", "chrome://drood/skin/splash/star.png");
  ...
  */
  ...
  // The next button
  if (getDroodService().notificationCount > 1) {
...
function droodDebugBreak () {
  // var breaker = Components.classes["@drood.com/debug-breaker;1"]
  //   .createInstance(Components.interfaces.nsIDroodDebugBreaker);
