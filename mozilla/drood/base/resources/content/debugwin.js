...
function promptForCloneLocation (title) {
  ...
  fp.defaultExtension = "drood";
  fp.defaultString = title + ".drood";
  if (fp.show() == IFilePicker.returnCancel)
    return null;
  if (! fp.file.leafName.match(/\.drood$/))
    fp.file.leafName += ".drood";
  return fp.file;
}
...
function cloneProject () {
  ...
  var clonefile = promptForCloneLocation(
    origfile.leafName.replace(/\.drood$/, "") + " (clone)");
  if (! clonefile)
    return;
  ...
  gWindow.projlist.view.setCellText(idx, secondCol,
    clonefile.leafName.replace(/\.drood$/, ""));
