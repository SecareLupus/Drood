...
var gScheduleUpdater = {
  ...
  onOperationComplete: function (aCalendar, aStatus, aOperationType,
    ...
    try {
      for (var i = 0; i < this.dirtyEvents.length; ++i) {
        ...
        var sceneuri = newEvent.getProperty("X-DROOD-SCENE");
        if (! sceneuri)
          continue;
  ...
  onGetResult: function (aCalendar, aStatus, aItemType, aDetail,
    ...
    for (var i = 0; i < aCount; ++i) {
      var event = aItems[i];
      if (! event.hasProperty("X-DROOD-SCENE"))
        continue;
      if (this.allScenesDirty) {
        this.dirtyEvents.push(event);
        continue;
      }
      var sceneuri = event.getProperty("X-DROOD-SCENE");
...
var gScheduleController = {
  ...
  open: function open (project, docres) {
    ...
    ps.addObserver("drood.calendar.workdaysonly", this, false);
    ps = ps.getBranch("drood.calendar.");
  ...
  locateShootStartAndEnd: function () {
    ...
    var listener = {
      onOperationComplete: function (aCalendar, aStatus, aOperationType,
        ...
        // Fix up Day Off events without the X-DROOD-DAYOFF property
        for (var i = 0; i < self._daysOff.length; ++i) {
          if (! self._daysOff[i].hasProperty("X-DROOD-DAYOFF")) {
            var newevent = self._daysOff[i].clone();
            newevent.setProperty("X-DROOD-MOVING", "TRUE");
        ...
        for (var i = 0; i < aCount; ++i) {
          ...
          if (event.hasProperty("X-DROOD-SHOOT-START")) {
            ...
            self._daysOff.push(event);
                  ...
                  ! event.hasProperty("X-DROOD-MOVING")) {
  ...
  editShootDays: function (title, msg) {
    ...
    try {
      if (this.startShootEvent) {
        ...
        event.setProperty("X-DROOD-SHOOT-START", "TRUE");
        event.startDate = config.startdate;
        event.endDate = event.startDate.clone();
        event.endDate.addDuration(oneday);
      ...
      if (this.endShootEvent) {
        ...
        event.setProperty("X-DROOD-SHOOT-END", "TRUE");
        event.startDate = config.enddate;
        event.endDate = event.startDate.clone();
        event.endDate.addDuration(oneday);
        doTransaction('add', event, this.calendar, null, null);
  ...
  close: function close () {
    ...
    var ps = getPrefService().QueryInterface(
      Components.interfaces.nsIPrefBranch2);
    ps.removeObserver("drood.calendar.workdaysonly", this);
    try {
  ...
  observe: function observe (subject, topic, data) {
    var ps = getPrefService().getBranch("drood.calendar.");
    var workdaysOnly = ps.getBoolPref("workdaysonly");
    gWindow.monthview.workdaysOnly = workdaysOnly;
    gWindow.weekview.workdaysOnly = workdaysOnly;
    gWindow.dayview.workdaysOnly = workdaysOnly;
  ...
  onSceneDblClick: function onSceneDblClick (event) {
    ...
    if (events) {
      ...
      for (var i = 0; i < events.length; ++i) {
        if (! events[i].hasProperty("X-DROOD-COMPLETED")) {
          completed = false;
          break;
        }
      }
      ...
      onGetResult: function (aCalendar, aStatus, aItemType, aDetail,
        ...
        for (var i = 0; i < aCount; ++i) {
          try {
            ...
            if (! item.hasProperty("X-DROOD-SCENE"))
              continue;

            var sceneid = item.getProperty("X-DROOD-SCENE");
            if (! sceneid)
              continue;
        ...
        for (var i = 0; i < scenes.length; ++i) {
          ...
          if (entry) {
            ...
            for (var j = 0; j < entry.length; ++j) {
              if (! entry[j].hasProperty("X-DROOD-COMPLETED")) {
  ...
  onModifyItem: function (item, oldItem) {
    ...
    if (item.hasProperty("X-DROOD-SHOOT-START") ||
        item.hasProperty("X-DROOD-SHOOT-END")) {
      if (item.hasProperty("X-DROOD-SHOOT-START")) {
    ...
    else if (item.hasProperty("X-DROOD-DAYOFF")) {
    ...
    var sceneid = oldItem.getProperty("X-DROOD-SCENE");
    if (! sceneid)
      return;
    ...
    sceneid = item.getProperty("X-DROOD-SCENE");
    if (! sceneid) {
    ...
    mapentry.push(item);
    for (var i = 0; i < mapentry.length; ++i) {
      if (! mapentry[i].hasProperty("X-DROOD-COMPLETED"))
        shouldBeComplete = false;
    }
    var ds = this.ds;
    var rdfsvc = getRDFService();
  ...
  onAddItem: function (item) {
    ...
    ds.beginUpdateBatch();
    setRDFString(ds, scene, scheduledarc, "true");
    if (! item.hasProperty("X-DROOD-COMPLETED"))
      setRDFString(ds, scene, completedarc, "false");
    ds.endUpdateBatch();
  ...
  onDragStart: function (event, xferData, action) {
    if (event.originalTarget.localName == "treechildren") {
      ...
      data.addDataForFlavour("x-drood/x-scene", scene.Value);
      xferData.data = data;
      return;
    }
    ...
    if (target.id == "movingitem") {
      ...
      xferData.data.addDataForFlavour("x-drood/x-banner", "moving");
    }
    else if (target.id == "dayoffitem") {
      xferData.data = new TransferData();
      xferData.data.addDataForFlavour("x-drood/x-banner", "dayoff");
    }
    else {
  ...
  decorateEventForScene: function (event, sceneuri, suppressModifyEvent) {
    ...
    var eventsceneuri = event.getProperty("X-DROOD-SCENE");
    var eventintext = event.getProperty("X-DROOD-INTEXT");
    var eventdaynight = event.getProperty("X-DROOD-DAYNIGHT");
    var eventsetting = event.getProperty("X-DROOD-SETTING");
    var eventord = event.getProperty("X-DROOD-ORDINAL");
    var eventeighths = event.getProperty("X-DROOD-EIGHTHS");
    ...
    event.setProperty("X-DROOD-SCENE", sceneuri);
    event.setProperty("X-DROOD-INTEXT", intext);
    event.setProperty("X-DROOD-SETTING", setting);
    event.setProperty("X-DROOD-DAYNIGHT", daynight);
    event.setProperty("X-DROOD-ORDINAL", ord);
    event.setProperty("X-DROOD-EIGHTHS", eighths);
  ...
  createEventWithDialog: function (aCalendar, aStartDate, aEndDate) {
    ...
    if (! aEndDate) {
      ...
      var pref = getPrefService().getBranch("drood.calendar.");
...
function showError(aMsg) {
  ...
  droodBugAlert(msg, Components.stack.caller, aMsg); 
}
