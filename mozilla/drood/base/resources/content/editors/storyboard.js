...
var gController = {
  ...
  open: function open (project, docres) {
    ...
    obsvc.addObserver(this, "drood:shot-document-saved", false);
  },


  close: function close () {
    var obsvc = getObserverService();
    obsvc.removeObserver(this, "drood:shot-document-saved");
  },
  ...
  observe: function (aSubject, aTopic, aData) {
    if (aTopic != "drood:shot-document-saved")
      return;
  ...
  refresh: function refresh () {
    ...
    var shotwords = getPrefString(
      getPrefService().getBranch("drood.scripteditor."), "shots")
      .split(",").join("|") + "|";
  ...
  sequenceForCell: function sequenceForCell (cell) {
    if (! cell) {
      droodBugAlert("Null cell in sequenceForCell", Components.stack, null);
      return null;
    }
  ...
  shotForCell: function shotForCell (cell) {
    if (! cell) {
      droodBugAlert("Null cell in shotForCell", Components.stack, null);
      return null;
    }
    var seqnum = cell.getAttribute("seqnum");
...
StoryboardTree.prototype = {
  ...
  onDragStart: function onDragStart (aEvent, aXferData, aDragAction) {
    ...
    data.addDataForFlavour("x-drood/x-treerow", this.selection.currentIndex);
    aXferData.data = data;
  },
  ...
  canDrop: function (row, orientation) {
    ...
    if (dragSession.numDropItems != 1 ||
        ! dragSession.isDataFlavorSupported("x-drood/x-treerow"))
      return false;

    var trans = Components.classes["@mozilla.org/widget/transferable;1"]
      .createInstance(Components.interfaces.nsITransferable);
    trans.addDataFlavor("x-drood/x-treerow");
    dragSession.getData(trans, 0);
    var data = {};
    var len = {};
    trans.getTransferData("x-drood/x-treerow", data, len);
