...
var gScriptController = {
  ...
  scriptWillLoad: function () {
    gPaginator = Components.classes["@drood.com/scriptpaginator;1"]
  ...
  scriptLoaded: function scriptLoaded () {
    ...
    if (this.mode == "theatre") {
      this.sceneTracker = Components.classes["@drood.com/acttracker;1"]
        .createInstance(Components.interfaces.nsISceneTracker);
    }
    else if (this.mode == "av" || this.mode == "comic") {
      this.sceneTracker = Components.classes["@drood.com/shottracker;1"]
        .createInstance(Components.interfaces.nsISceneTracker);
    }
    else {
      this.sceneTracker = Components.classes["@drood.com/scenetracker;1"]
  ...
  breakdownUnitContainingNode: function (aNode) {
    ...
    var IPara = Components.interfaces.nsIDOMHTMLParagraphElement;
    var iter = Components.classes["@drood.com/dom/iterator;1"]
      .createInstance(Components.interfaces.droodINodeIterator);
  ...
  hideUnusedFormats: function hideUnusedFormats () {
    try {
      ...
      var ps = getPrefService().getBranch("drood.scripteditor."
        + this.mode + ".");
  ...
  open: function open (project, docres) {
    ...
    this.docres = docres;
    this.document = new DroodDocument();
    this.document.init(project, docres);

    var obs = getObserverService();
    obs.addObserver(this, "drood:script:will-change-lock", false);
    obs.addObserver(this, "drood:script:did-change-lock", false);

    var ps = getPrefService().QueryInterface(
      Components.interfaces.nsIPrefBranch2);
    ps.addObserver("drood.script.breakdown.", this, false);
  ...
  close: function close () {
    ...
    var ps = getPrefService().QueryInterface(
      Components.interfaces.nsIPrefBranch2);
    ps.removeObserver("drood.script.breakdown.", this);
  ...
  observe: function (aSubject, aTopic, aData) {
    if (aTopic == "nsPref:changed") {
      if (aData == "drood.script.breakdown.visible")
    ...
    if (aTopic == "drood:script:will-change-lock") {
      if (aData == "locked") {
        this.editor.editor.removeInsertionListener(this);
        this.editor.editor.transactionManager.RemoveListener(this);
        InlineSpellChecker.shutdown();
    ...
    else if (aTopic == "drood:script:did-change-lock") {
      if (aData == "unlocked") {
        this.editor.editor.addInsertionListener(this);
        this.editor.editor.transactionManager.AddListener(this);
        InlineSpellChecker.Init(this.editor.editor);
  ...
  saveCursorLocation: function saveCursorLocation () {
    ...
    var pref = getPrefService().getBranch("drood.project.");
  ...
  updateSidebarStatus: function () {
    ...
    var toolbaritem = document.getElementById("drood-sidebar-button");
    if (menuitem)
  ...
  updateAutoText: function updateAutoText () {
    ...
    var ps = getPrefService().getBranch("drood.scripteditor.autocomplete.");
    try {
  ...
  showBreakdownChanged: function () {
    var prefs = getPrefService().getBranch("drood.script.breakdown.");
  ...
  getSceneNumberingScheme: function () {
    ...
    if (! scheme) {
      // Set a default one (TODO: Use Drood style when non-English?)
  ...
  cmdResetLocking: function () {
    ...
    try {
      ...
      droodBugAlert(ex, Components.stack, "Reseting scene numbers failed");
    }
    ...
    try {
      ...
      droodBugAlert(ex, Components.stack, "Removing revision marks failed");
    }
    ...
    try {
      ...
      droodBugAlert(ex, Components.stack, "Restoring omitted scenes failed");
    }
...
var InlineSpellChecker = {
  ...
  Init: function (editor) {
    ...
    ps.addObserver("drood.spelling.", this, false);
  ...
  shutdown: function () {
    var ps = getPrefService();
    ps = ps.QueryInterface(Components.interfaces.nsIPrefBranch2);
    ps.removeObserver("drood.spelling.", this, false);
  },

  observe: function (subject, topic, data) {
    if (topic != "nsPref:changed" || data != "drood.spelling.inline")
      return;

    var ps = Components.classes["@mozilla.org/preferences-service;1"]
      .getService(Components.interfaces.nsIPrefService)
      .getBranch("drood.spelling.");
...
function PageConfig (doc, editortype) {
  ...
  var IMeta = Components.interfaces.nsIDOMHTMLMetaElement;
  var stylePrefix = "chrome://drood/content/style/";
  ...
  for (var i = 0; i < children.length; ++i) {
    ...
    if (child instanceof ILink) {
      if (child.href == "chrome://drood/content/editor.css") {
        editorLink = child;
        continue;
      }
 ...
 * that depended on the memory layout of nsCocoaWindow. In Drood 2.9, the
 * layout changed because nsBaseWidget (an ancestor class of nsCocoaWindow)
 * added an extra member variable.
 *
 * Drood 2.9 ships with a built-in replacement for the compiled component so
...
function fullscreen_toggleFullScreen2 () {
  ...
  obsvc.notifyObservers(window, "drood:will-change-fullscreen", ! gFullScreen);
  ...
  if (isMac()) {
    try {
      if ("@drood.com/fullscreen;2" in Components.classes)
        fullscreenComponent = Components.classes["@drood.com/fullscreen;2"]
    ...
    try {
      if (! fullscreenComponent &&
          "@drood.com/fullscreen;1" in Components.classes)
        fullscreenComponent = Components.classes["@drood.com/fullscreen;1"]
          .createInstance(Components.interfaces.nsICocoaFullScreen);
    }
    catch (ex) {
  ...
  obsvc.notifyObservers(window, "drood:did-change-fullscreen", ! gFullScreen);
...
window.addEventListener("load", function () {
  ...
  var item = extmgr.getItemForID("fullscreen@drood.com");
