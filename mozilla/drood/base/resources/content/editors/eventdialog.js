...
function onLoad() {
  ...
  if (gWindow.calendarItem.hasProperty("X-DROOD-SCENE")) {
    gIsScene = true;
    gWindow.allDay.hidden = true;
    document.getElementById("scenedeck").selectedIndex = 1;
    var rdfsvc = getRDFService();
    var sceneuri = gWindow.calendarItem.getProperty("X-DROOD-SCENE");
  ...
  else if (gWindow.calendarItem.hasProperty("X-DROOD-DAYOFF") ||
           gWindow.calendarItem.hasProperty("X-DROOD-MOVING") ||
           gWindow.calendarItem.hasProperty("X-DROOD-SHOOT-START") ||
           gWindow.calendarItem.hasProperty("X-DROOD-SHOOT-END"))
    gWindow.allDay.hidden = true;
...
function loadDialog (item) {
  if (gIsScene) {
    ...
    var rdfsvc = getRDFService();
    var sceneuri = item.getProperty("X-DROOD-SCENE");
    var sceneres = rdfsvc.GetResource(sceneuri);
...
function saveDialog (item) {
  ...
  if (gWindow.sceneCompleted.checked)
    setItemProperty(item, "X-DROOD-COMPLETED", "YES");
  else
    item.deleteProperty("X-DROOD-COMPLETED");
