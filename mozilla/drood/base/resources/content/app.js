...
var gApp = {
  ...
  getText: function (name, params) {
    if (! this.sb) {
      ...
      var bundlePath = Cx.LOCALE_PATH + "drood.properties";
  ...
  doCommand: function (cmd) {
    ...
    switch (cmd) {
      ...
      case "cmd-open-studio":
        // this.openBrowser(getDroodService().STUDIO_BASEURL + "/");
        this.openStudioProject();
        break;
      case "cmd-about-drood":
  ...
  setToolbarMode: function setToolbarMode (mode) {
    var ps = getPrefService().getBranch("drood.toolbar.");
    ps.setCharPref("show", mode);
  },


  observe: function (subject, topic, data) {
    if (topic != "nsPref:changed" || data != "drood.toolbar.show")
      return;
    var ps = getPrefService().getBranch("drood.toolbar.");
  ...
  init: function init () {
    ...
    ps.addObserver("drood.toolbar.", this, false);
    this.initPrefs();
    ...
    getPrefService().getBranch("drood.").setCharPref("version", Cx.VERSION);
    ...
    if (dstdir.exists()) {
      if (! forceupdate) {
        ...
        while (entries.hasMoreElements()) {
          ...
          if (entry.leafName.match(/\.t?drood$/))
            return;
        }
      }
    }
  ...
  checkTemplates: function checkTemplates () {
    ...
    try {
      lastVersion = new Version(prefsvc.getCharPref("drood.version"));
    }
    catch (ex) {
    ...
    audiotmpl.append("4_Podcast.drood");
    if (audiotmpl.exists()) {
      ...
      srcfile.append("4_Podcast.drood");
      srcfile.copyTo(dstfile, srcfile.leafName);
    }

    // Renumber the Text project from 7 to 6
    var txttmpl = dstfile.clone();
    txttmpl.append("6_Text.tdrood");
    if (txttmpl.exists()) {
      try {
        txttmpl.moveTo(txttmpl.parent, "7_Text.drood");
      }
      catch (ex) {
        dump("*** Error moving 6_Text.tdrood to 7_Text.drood: " + ex + "\n");
      }
    }

    var oldcomic = dstfile.clone();
    oldcomic.append("7_ComicBook.drood");
    if (oldcomic.exists()) {
      try {
        oldcomic.remove(false);
      }
      catch (ex) {
        dump("*** Error deleting 7_ComicBook.drood: " + ex + "\n");
      }
    }

    var dsticon = dstfile.clone();
    dstfile.append("6_ComicBook.drood");
    ...
    srcfile.append("6_ComicBook.drood");
  ...
  checkSuperbundle: function checkSuperbundle () {
     ...
     * install file by replacing .xpi with @drood.com, e.g.,
     *     art-pack-1.xpi -> art-pack-1@drood.com
     *
     * That avoids needing to unpack and analyze every extension,
     * every time Drood is launched.
     */
    ...
    var installing = true;
    try {
      installing = prefsvc.getBranch("drood.superbundle.").getBoolPref("installing");
    }
    catch (ex) {
      dump("*** Couldn't check drood.superbundle.installing pref");
    }
    ...
    while (files.hasMoreElements()) {
      ...
      if (! extfile.leafName.match(/\.(xpi|cxext)$/))
        continue;
      var extid = extfile.leafName.replace(/\.(xpi|cxext)$/, "@drood.com");
      try {
    ...
    if (newInstalls && ! installing) {
      try {
        prefsvc.getBranch("drood.superbundle.").setBoolPref("installing", true);
    ...
    prefsvc.getBranch("drood.superbundle.").setBoolPref("installing", false);
  },
  ...
  initPrefs: function initPrefs () {
    ...
    try {
      ...
      if (prefsvc.prefHasUserValue(cookiepref)) {
        ...
        prefsvc.setBoolPref("drood.server.ping", false);
    ...
    try {
      var clearpass = prefsvc.getCharPref("drood.user.password");
      if (clearpass) {
        prefsvc.setCharPref("drood.user.encpassword",
          base64_encodew(clearpass));
        prefsvc.clearUserPref("drood.user.password");
      }
    }
    catch (ex) {}
    ...
    try {
      firstLaunch = prefsvc.getBoolPref("drood.firstlaunch");
      if (firstLaunch) {
        prefsvc.setBoolPref("drood.firstlaunch", false);
        firstLaunch = false;
      }
    }
    catch (ex) {
      prefsvc.setBoolPref("drood.firstlaunch", true);
    }
    ...
    try {
      this.setToolbarMode(prefsvc.getCharPref("drood.toolbar.show"));
    }
    catch (ex) {
    ...
    this.observe(prefsvc, "nsPref:changed", "drood.toolbar.show");

    try {
      var dir = prefsvc.getCharPref("drood.projectsdirectory");
    }
    catch (ex) {
      try {
        dir = userDocsDir().path;
        prefsvc.setCharPref("drood.projectsdirectory", dir);
      }
      catch (ex2) {
  ...
  findProjectWindowByFile: function findProjectWindowByFile (file) {
    ...
    var e  = wm.getEnumerator('drood:main');
    while (e.hasMoreElements()) {
      var w = e.getNext();
      var droodfile = w.getProjectFile();
      if (droodfile && droodfile.equals(file)) {
        return w;
      }
    }
  ...
  openProject: function openProject () {
    ...
    fp.appendFilter(gApp.getText("DroodProject"), "*.drood");
    // fp.appendFilter(gApp.getText("DroodTemplate"), "*.tdrood");
    fp.displayDirectory = getDroodProjectsDir();
    if (fp.show() != IFilePicker.returnOK)
      return false;

    if (! (fp.file.leafName.match(/\.t?drood$/))) {
      alert(gApp.getText("InvalidProjectFile"));
      return false;
    }
    var spec = fp.fileURL.spec;

    setDroodProjectsDir(fp.file.parent);
  ...
  cmdAccountWizard: function cmdAccountWizard () {
    ...
    gApp.openBrowser(getDroodService().STUDIO_BASEURL + "/");
  }
};
...
function addToRecentProjects (filedesc) {
  if (! filedesc) {
    droodBugAlert("fileuri is undefined", Components.stack);
    return;
  }
  ...
  getObserverService().notifyObservers(gProjMgr,
    "drood:recent-projects-changed", null);
}
...
function showSplashWindow () {
  ...
  var splashes = med.getEnumerator("drood:splash");
