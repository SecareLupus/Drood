...
/*
 * This file consolidates all the code related to saving a Drood project.
 *
...
function savelogmsg (aMsg, aFlush) {
  if (! gSaveLogger) {
    var logsvc = Components.classes["@drood.com/log-service;1"]
      .getService(Components.interfaces.droodILogService);
...
function cmdSaveProjectAsFile (callback) {
  ...
  var location = promptForFileSaveLocation(getDroodProjectsDir(), false);
  if (location) {
    savelogmsg("  Received user location: " + location.path);
    setDroodProjectsDir(location.parent);
    gProject.title = location.leafName.replace(/\.drood$/, "");
...
function promptForFileSaveLocation (dir, template) {
  var defaultext = template ? "tdrood" : "drood";
  var typename = gApp.getText(template ? "DroodTemplate" : "DroodProject");
...
function doSaveProjectToTemplate (file, callback) {
  ...
  gProject.title = file.leafName.replace(/\.t?drood$/, "");
  saveProjectToFile(file, innercb);
}
...
function doSaveProjectToServer (wsref, callback) {
  ...
  function doSaveProjectToServer_step (step) {
    ...
    if (step == "start") {
      ...
      var projuri = getDroodService().workspaceURI + "/project/" + wsref;
      ...
      var pref = getPrefService().getBranch("drood.server.");
  ...
  try {
    var innercb = {
      succeeded: function () {
        ...
        obsvc.notifyObservers(gProject, "drood:project-saved", null);
      },
      cancelled: function () {
    ...
    archiveDroodProject(file, innercb);
  }
  catch (ex) {
...
function saveScriptToServer (callback) {
  try {
    ...
    var contentType = "application/x-drood-script+xml; "
      + "droodType=" + scriptType;
...
/**
 * Writes the Drood project to a zip file.
 */
function archiveDroodProject (file, callback) {
  savelogmsg("archiveDroodProject");
  ...
  while (entries.hasMoreElements()) {
    ...
    if (! entry.isFile() || entry.leafName.match(/\.t?drood$/)) {
