...
NS_IMPL_ISUPPORTS1(droodNodeIterator, droodINodeIterator)

droodNodeIterator::droodNodeIterator ()
: mCurrentNode(nsnull), mRange(nsnull), mFirst(nsnull), mLast(nsnull)
{
}

/* void init (in nsIDOMNode start, in nsIDOMNode range); */
NS_IMETHODIMP droodNodeIterator::Init(nsIDOMNode *start, nsIDOMRange* range)
{
...
/* nsIDOMNode nextNode (); */
NS_IMETHODIMP droodNodeIterator::NextNode(nsIDOMNode **_retval)
{
...
NS_IMETHODIMP droodNodeIterator::PreviousNode(nsIDOMNode **_retval)
{
  NS_ENSURE_TRUE(mFirst, NS_ERROR_NOT_INITIALIZED);
  NS_ENSURE_ARG_POINTER(_retval);
