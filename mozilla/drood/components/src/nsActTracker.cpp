...
NS_IMETHODIMP nsActTracker::Init (nsIRDFDataSource* ds,
  ...
  mRDFSvc->GetResource(NS_LITERAL_CSTRING("http://drood.com/NS/v1/acts"),
  ...
  rv = DroodRDFUtils::GetRDFSeq(mDS, actsres, getter_AddRefs(mActs));
  if (NS_FAILED(rv))
    return rv;
...
NS_IMETHODIMP nsActTracker::Update () {
  ...
  mRDFSvc->GetResource(
    NS_LITERAL_CSTRING("http://drood.com/NS/v1/actid"),
  ...
  for (PRUint32 i = 0; i < acts.Length(); ++i) {
    ...
    rv = DroodRDFUtils::GetRDFSeq(mDS, actres, getter_AddRefs(actScenes));
    if (NS_FAILED(rv))
      return rv;
...
nsActTracker::ActForActID (const nsAString& actid,
                             nsIRDFResource** result) {
  nsCOMPtr<nsIRDFResource> actidarc;
  mRDFSvc->GetResource(NS_LITERAL_CSTRING("http://drood.com/NS/v1/actid"),
    getter_AddRefs(actidarc));
  nsCOMPtr<nsIRDFLiteral> idlit;
