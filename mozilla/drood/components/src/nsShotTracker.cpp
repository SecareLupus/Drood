...
NS_IMETHODIMP nsShotTracker::Init (nsIRDFDataSource* ds,
  ...
  mRDFSvc->GetResource(NS_LITERAL_CSTRING("http://drood.com/NS/v1/shots"),
    getter_AddRefs(shotsarc));
  nsCOMPtr<nsIRDFResource> scenesarc;
  mRDFSvc->GetResource(NS_LITERAL_CSTRING("http://drood.com/NS/v1/scenes"),
  ...
  rv = DroodRDFUtils::GetRDFSeq(mDS, shotsres, getter_AddRefs(mShots));
  if (NS_FAILED(rv))
    return rv;
  ...
  rv = DroodRDFUtils::GetRDFSeq(mDS, scenesres, getter_AddRefs(mScenes));
  if (NS_FAILED(rv))
    return rv;
...
NS_IMETHODIMP nsShotTracker::Update () {
  ...
  mRDFSvc->GetResource(
    ...
    getter_AddRefs(titlearc));
  mRDFSvc->GetResource(
    NS_LITERAL_CSTRING("http://drood.com/NS/v1/ordinal"),
      getter_AddRefs(ordarc));
  mRDFSvc->GetResource(
    NS_LITERAL_CSTRING("http://drood.com/NS/v1/sortord"),
      getter_AddRefs(sortordarc));
  mRDFSvc->GetResource(
    NS_LITERAL_CSTRING("http://drood.com/NS/v1/shotid"),
      getter_AddRefs(shotidarc));
  mRDFSvc->GetResource(
    NS_LITERAL_CSTRING("http://drood.com/NS/v1/sceneid"),
      getter_AddRefs(sceneidarc));
  mRDFSvc->GetResource(
    NS_LITERAL_CSTRING("http://drood.com/NS/v1/intext"),
      getter_AddRefs(intextarc));
  mRDFSvc->GetResource(
    NS_LITERAL_CSTRING("http://drood.com/NS/v1/setting"),
      getter_AddRefs(settingarc));
  mRDFSvc->GetResource(
    NS_LITERAL_CSTRING("http://drood.com/NS/v1/daynight"),
    getter_AddRefs(daynightarc));
  mRDFSvc->GetResource(
    NS_LITERAL_CSTRING("http://drood.com/NS/v1/location"),
  ...
  for (PRUint32 i = 0; i < scenes.Length(); ++i) {
    ...
    if (NS_FAILED(rv) || ! sceneres) {
      nsCString mintedURI(NS_LITERAL_CSTRING("http://drood.com/res/"));
      nsCString genid;
      GenerateID(genid);
    ...
    rv = DroodRDFUtils::GetRDFSeq(mDS, sceneres, getter_AddRefs(sceneShots));
    if (NS_FAILED(rv))
      return rv;
...
nsShotTracker::ShotForShotID (const nsAString& shotid,
  ...
  nsCOMPtr<nsIRDFResource> shotidarc;
  mRDFSvc->GetResource(NS_LITERAL_CSTRING("http://drood.com/NS/v1/shotid"),
    getter_AddRefs(shotidarc));
...
nsShotTracker::SceneForSceneID (const nsAString& sceneid,
                                nsIRDFResource** result) {
  nsCOMPtr<nsIRDFResource> sceneidarc;
  mRDFSvc->GetResource(NS_LITERAL_CSTRING("http://drood.com/NS/v1/sceneid"),
    getter_AddRefs(sceneidarc));
  nsCOMPtr<nsIRDFLiteral> idlit;
...
nsresult nsShotTracker::SplitHeading (const nsAString& aHeading,
  ...
  nsresult rv = ps->GetBranch("drood.scripteditor.", getter_AddRefs(branch));
  if (NS_FAILED(rv))
    return rv;
