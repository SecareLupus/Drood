...
NS_IMETHODIMP nsSceneTracker::Init (nsIRDFDataSource* ds,
  ...
  mNumberSvc = do_GetService("@drood.com/scene-number-service;1");
...
nsresult nsSceneTracker::CheckNumberingScheme () {
  ...
  mRDFSvc->GetResource(NS_LITERAL_CSTRING(
    "http://drood.com/NS/v1/sceneNumberScheme"), getter_AddRefs(schemearc));
  ...
  if (! schemeres)
    mRDFSvc->GetResource(NS_LITERAL_CSTRING(
      "http://drood.com/NS/v1/HollywoodNumberScheme"),
...
nsresult nsSceneTracker::UpdateSceneNumberLiterals (
  ...
  mRDFSvc->GetResource(NS_LITERAL_CSTRING("http://drood.com/NS/v1/ordinal"),
    getter_AddRefs(ordarc));
  mRDFSvc->GetResource(NS_LITERAL_CSTRING("http://drood.com/NS/v1/sortord"),
