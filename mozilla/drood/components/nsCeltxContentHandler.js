...
const nsIDroodHandler        = Components.interfaces.nsIDroodHandler;
...
function shouldLoadURI(aURI) {
  // Drood supports file: and drood: schemes. That's it.
  if (aURI && (aURI.schemeIs("file") || aURI.schemeIs("drood") ||
    aURI.schemeIs("droods")))
    return true;

  dump("*** Preventing external load of " + aURI.scheme
    + " URI into Drood window\n");
  return false;
}
...
function openPreferences() {
  ...
  var url = "chrome://drood/content/preferences/preferences.xul";

  var win = getMostRecentWindow("Drood");
  if (win) {
    win.focus();
  } else {
...
// this returns the most recent Drood window
function getMostRecentDroodWindow() {
  var wm = Components.classes["@mozilla.org/appshell/window-mediator;1"]
                     .getService(Components.interfaces.nsIWindowMediator);

#ifdef BROKEN_WM_Z_ORDER
  var win = wm.getMostRecentWindow("drood:main", true);

  // if we're lucky, this isn't a popup, and we can just return this
  if (win && !win.toolbar.visible) {
    var windowList = wm.getEnumerator("drood:main", true);
...
#else
  var windowList = wm.getZOrderDOMWindowEnumerator("drood:main", true);
...
var nsDroodContentHandler = {
  ...
  handle : function cch_handle(cmdLine) {
    if (cmdLine.handleFlag("drood", false)) {
      openWindow(null, "chrome://drood/content/templates.xul", "_blank",
  ...
  helpInfo : "  -drood            Open a Drood window.\n",
...
const cch_contractID = "@drood.com/drood/clh;1";
...
var nsDefaultCommandLineHandler = {
    ...
    if (urilist.length) {
      ...
      for (var i = 0; i < speclist.length; ++i) {
        ...
        if (! (fileurl instanceof IFileURL)) {
          openWindow(null, "chrome://drood/content/", "_blank",
  ...
  openFile: function (aFile) {
    ...
    var windows = wm.getEnumerator("drood:main");
    while (windows.hasMoreElements()) {
      var win = windows.getNext();
      var droodfile = win.getProjectFile();
      if (droodfile && droodfile.equals(aFile)) {
        win.focus();
        return;
      }
    }
    ...
    openWindow(null, nsDroodContentHandler.chromeURL, "_blank",
      "chrome,dialog=no,all", fileurl);

    var splashes = wm.getEnumerator("drood:splash");
    while (splashes.hasMoreElements())
      splashes.getNext().close();
  },

  // XXX localize me... how?
  helpInfo : "Usage: drood [-flags] [<url>]\n",
...
const dch_contractID = "@drood.com/drood/final-clh;1";
...
var Module = {
  ...
  getClassObject: function mod_getco(compMgr, cid, iid) {
    if (cid.equals(cch_CID))
      return nsDroodContentHandler.QueryInterface(iid);
  ...
  registerSelf: function mod_regself(compMgr, fileSpec, location, type) {
    if (Components.classes["@mozilla.org/xre/app-info;1"]) {
      // Don't register these if Drood is launching a XULRunner application
      const DROOD_UID = "drood@drood.com";
      var appInfo = Components.classes["@mozilla.org/xre/app-info;1"]
                              .getService(Components.interfaces.nsIXULAppInfo);
      if (appInfo.ID != DROOD_UID)
        return;
    }
    ...
    registerType("application/x-drood-project");
    registerType("application/x-drood-extension");
