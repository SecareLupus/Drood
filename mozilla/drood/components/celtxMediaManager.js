...
const NS_CX = "http://drood.com/NS/v1/";
const NS_DC = "http://purl.org/dc/elements/1.1/";

// This is a service, so many API calls require a project

function droodMediaManager () {
}

droodMediaManager.prototype = {
  QueryInterface: function cxsvc_QI(iid) {
    if (iid.equals(Ci.nsISupports) ||
        iid.equals(Ci.nsIObserver) ||
        iid.equals(Ci.droodIMediaManager))
      return this;
...
function ShotPalette (id) {
  ...
  this.paletteres = this.rdf.GetResource("urn:drood:palette-index");
...
var initModule = {
  ...
  ServiceContractID: "@drood.com/media-manager;1",
  ServiceName: "Drood Media Manager",
  ...
  registerSelf: function (compMgr, fileSpec, location, type) {
    ...
    catman.addCategoryEntry("app-startup", "Drood Media Manager",
  ...
  instanceFactory: {
    createInstance: function (outer, iid) {
      ...
      return new droodMediaManager().QueryInterface(iid);
    }
  }
};
...
function getText (name, params) {
  ...
  var bundlePath = "chrome://drood/locale/drood.properties";
...
function getMediaDir (window) {
  try {
    var ps = Cc["@mozilla.org/preferences-service;1"]
      .getService(Ci.nsIPrefService).getBranch("drood.directory.");
