...
nsDroodService.prototype = {
  QueryInterface: function cxsvc_QI(iid) {
    if (iid.equals(Ci.nsISupports) ||
        iid.equals(Ci.nsIObserver) ||
        iid.equals(Ci.nsIDroodService))
      return this
  ...
  get PUBLISH_SERVER () {
    ...
    return ps.getBranch("drood.server.").getCharPref("publish.selection");
  },
  get STARTUP_URL () {
  ...
  get STUDIO_SERVER () {
    var ps = Cc["@mozilla.org/preferences-service;1"]
      .getService(Ci.nsIPrefService);
    return ps.getBranch("drood.server.").getCharPref("studio.selection");
  },
  get STUDIO_SCHEME () {
    var ps = Cc["@mozilla.org/preferences-service;1"]
      .getService(Ci.nsIPrefService);
    return ps.getBranch("drood.server.").getCharPref("studio.scheme");
  },
  get STUDIO_BASEURL () {
  ...
  get PDF_SERVER () {
    var ps = Cc["@mozilla.org/preferences-service;1"]
      .getService(Ci.nsIPrefService);
    return ps.getBranch("drood.server.").getCharPref("render.selection");
  },
  get PDF_CONVERT_URL () {
  ...
  startup: function startup () {
    // If drood.server.ping is false, this will not store the stats cookie
  ...
  shutdown: function shutdown () {
    try {
      ...
      dump("*** nsDroodService.shutdown: " + ex + "\n");
    }
  ...
  set offline (val) {
    if (this._offline != val) {
      ...
      var os = Cc["@mozilla.org/observer-service;1"]
        .getService(Ci.nsIObserverService);
      os.notifyObservers(this, "drood:network-status-changed",
        val ? "offline" : "online");
    }
    return val;
  },
  ...
  set banners (val) {
    ...
    var os = Cc["@mozilla.org/observer-service;1"]
      .getService(Ci.nsIObserverService);
    os.notifyObservers(this, "drood:banner-data-changed", null);
  },
    ...
    if (username && workspace) {
      ...
      if (session) {
        ...
        var obssvc = Cc["@mozilla.org/observer-service;1"]
          .getService(Ci.nsIObserverService);
        obssvc.notifyObservers(this, "drood:login-status-changed", "loggedin");
      }
    }
    else {
      dump("*** checkLoginFromCookie: username = " + username
  ...
  checkAutoLogin: function (win) {
    ...
    var ps = Cc["@mozilla.org/preferences-service;1"]
      .getService(Ci.nsIPrefService).getBranch("drood.");
  ...
  attemptLogin: function (observer, win, logindata) {
    if (logindata.prompt) {
      ...
      win.openDialog("chrome://drood/content/authenticate.xul", "",
    ...
    request.onload  = function () {
      ...
      // If unauthorized, only reattempt if we prompted for login info
        ...
        // Not Authorized
        dump("*** nsDroodService.login: " + request.status + " "
          + request.statusText + "\n");
        logindata.message = "";
        logindata.location = "";
        ...
        dump("*** nsDroodService.login: status: " + request.status + "\n");
        if (observer)
          observer.onLogin(false);
      }
    };
  ...
  logout: function cxsvc_logout () {
    ...
    var obssvc = Cc["@mozilla.org/observer-service;1"]
      .getService(Ci.nsIObserverService);
    obssvc.notifyObservers(this, "drood:login-status-changed", "loggedout");
...
var initModule = {
  ...
  ServiceContractID: "@drood.com/drood-service;1",
  ServiceName: "Drood Service",
  ...
  instanceFactory: {
    createInstance: function (outer, iid) {
      ...
      return new nsDroodService().QueryInterface(iid);
    }
  }
};
